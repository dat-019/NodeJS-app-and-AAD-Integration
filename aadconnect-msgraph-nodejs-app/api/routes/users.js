...
/*  Filtering user by the user's country/region 
  Ex: http://localhost:3000/users/filterUsersWithCountry/Germany
*/
router.get('/filterUsersWithCountry/:country',
  async function (req, res, next) {
    var userList = [];
    if (!req.isAuthenticated()) {
      res.redirect('/auth/signin'); // if the request is unauthenticated, redirect to login page
      ...
      if (accessToken && accessToken.length > 0) {
        var p_Country = req.params.country;
        var requestUrl = "https://graph.microsoft.com/v1.0/users?$top=999&$filter=country eq '" + p_Country + "'";
        try {
          do {
